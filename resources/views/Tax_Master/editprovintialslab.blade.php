                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="card-title d-flex align-items-center">
                                ...
                                <h5 class="mb-0 text-primary">Provintial Tax Slab</h5>
                            </div>
                            <hr>
                            <form class="row g-2" action="{{route('master.update_provitialslab')}}" method="post">	
                                @csrf
                            <input type="hidden" name="id" value="{{$editpd->id}}">
                                
                                <div class="col-md-2">
                                    <label for="inputFirstName" class="form-label">Min Value</label>
                                    <input type="text" class="form-control" id="inputFirstName" placeholder="Min Value" name="min_values" value="{{$editpd->min_values}}"> 
                                </div>
                                
                                <div class="col-md-2">
                                    <label for="inputFirstName" class="form-label">Max Value</label>
                                    <input type="text" class="form-control" id="inputFirstName" placeholder="Max Value" name="max_values"  value="{{$editpd->max_values}}"> 
                                </div>
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="text" class="form-control" id="inputFirstName" placeholder="Percentage of Tax" name="percentage_of_taxs" value="{{$editpd->percentage_of_taxs}}"> 
                                </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="example" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                @foreach ($pd as $protaxsalb)
              
                                <tr>
                                    <td>{{$loop->index+1}}</td>
                                    <td>{{$protaxsalb->min_values}}</td>
                                      <td>{{$protaxsalb->max_values}}</td>
                                     <td>{{$protaxsalb->percentage_of_taxs}}</td>
                                    ...
                                    <td style="background-color:#ffff;">
                                        ...
                                        <a href="{{route('master.edit_provitialslab',$protaxsalb->id)}}">
                                        <button type="button" class="btn1 btn-outline-primary"><i class='bx bx-edit-alt me-0'></i></button></a>
                                        <a href="{{route('master.destroy_provitialslab',$protaxsalb->id)}}">
